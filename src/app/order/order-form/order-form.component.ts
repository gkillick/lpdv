...
export class OrderFormComponent implements OnInit, OnChanges, OnDestroy {
  ...
  getPersonalData(form: FormGroup): CustomerFormData {
    const {first_name, last_name, telephone, date, timeOfDay, payed, notes } = form.value;
    return {first_name, last_name, telephone, date, sub_total: this.subTotal, tax: this.tax, total: this.total, timeOfDay, payed, notes};
  }
